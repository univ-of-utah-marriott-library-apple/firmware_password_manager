...
# ard_wrapped_remotely_set_firmwarepassword_scp_keyfile.sh #####################
#
# A bash script to demonstrate usage of Firmware Password Manager from Apple
# Remote Desktop.
#
#    1.0.0  2016.03.07      Initial release. tjm
#
# ARD will only run bash scripts from "Send UNIX Command", this example
# demonstrates wrapping a python script in bash.
#
################################################################################

function wrap {
PYTHON_ARG="$1" python - <<END
# cut or comment out #!/usr/bin/python
import subprocess
import os
import pexpect
...
def main():
    ...
    try:
        child = pexpect.spawn("/usr/bin/scp -P22 " + scp_user + "@" + scp_server + ":" + keyfile_remote_path + " " + keyfile_local_path)
